...
function plot_mag(xyz::XYZ;
    ...
    ax  = Axis(fig[1, 1], xlabel = "time [s]", ylabel = "magnetic field [nT]")
    ...
    # ... [Keep your existing field identification logic here] ...

    # Plotting logic
    if :comp_mags in use_mags
        for i in eachindex(mags_c_)
            val = (getfield(xyz, list_uc[mags_uc_[i]]) - getfield(xyz, list_c[mags_c_[i]]))[ind]
            # Manual detrending without Statistics module
            if detrend_data
                m = manual_mean(val)
                val = val .- m
            end
            lines!(ax, tt, val, label="mag_$i comp")
        end
    elseif any(use_mags .∈ (field_check(xyz, MagV),))
        for use_mag in use_mags[use_mags .∈ (field_check(xyz, MagV),)]
            ...
            current_vecs = :all in vec_terms ? [:x, :y, :z, :t] : vec_terms
            for vec_term in current_vecs
                val = getfield(flux, vec_term)
                if detrend_data
                    m = manual_mean(val)
                    val = val .- m
                end
                lines!(ax, tt, val, label="$use_mag $vec_term")
            end
        end
    else
        ...
        for mag in use_mags
            if mag in fields
                ...
                lines!(ax, tt, val, label=string(mag))
            end
        end
    end

    !isempty(ylim) && ylims!(ax, ylim...)
    axislegend(ax)

    show_plot && display(fig)
    save_plot && save(plot_png, fig)
